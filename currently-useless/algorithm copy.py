...
class Algorithm(Subject):
    def __init__(self, bounding_poly: Polygon = None, remove_zero_length_edges=True):
        ...
        # Zdarzenie obecne i struktura zdarzeń
        self.event_queue = PriorityQueue()
        self.event = None

        # root linii brzegowej
    ...
    def create_diagram(self, points: list):
        ...
        # inicjalizacja struktury zdarzeń
        ...
        while not self.event_queue.empty():
            ...
            # zdejmowanie z kolejki kolejnego zdarzenia
            event = self.event_queue.get()


            # przypadek gdy zdarzenie wyjętez kolejki jest zdarzeniem kołowym
            if isinstance(event, CircleEvent) and event.is_valid:

                # uaktualnienie miotły
                self.sweep_line = event.yd

                self.handle_circle_event(event)

            # przypadek gdy zdarzenie wyjęte z kolejki jest zdarzeniem punktowym
            elif isinstance(event, SiteEvent):

                # numerowanie punktów (dla ułatwienia)
                ...
                # pominięcie gdy zdarzenie jest zdarzeniem kołowym, ale jest nieaktualne 
                # czyli było wrzucone do kolejki ale okazało się "fałszywym alarmem"
                continue
